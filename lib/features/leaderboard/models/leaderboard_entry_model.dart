class LeaderboardEntryModel {
  ...
  factory LeaderboardEntryModel.fromJson(Map<String, dynamic> json) {
    return LeaderboardEntryModel(
      ...
      businessValue:
          double.tryParse(json['business_value']?.toString() ?? '0') ?? 0.0,
      inventoryValue:
          double.tryParse(json['inventory_value']?.toString() ?? '0') ?? 0.0,
      vehicleValue:
          double.tryParse(json['vehicle_value']?.toString() ?? '0') ?? 0.0,
      buildingBaseValue:
          double.tryParse(json['building_base_value']?.toString() ?? '0') ??
          0.0,
      buildingUpgradeValue:
          double.tryParse(json['building_upgrade_value']?.toString() ?? '0') ??
          0.0,
      warehouseInventoryValue:
          double.tryParse(
            json['warehouse_inventory_value']?.toString() ?? '0',
          ) ??
          0.0,
      storeInventoryValue:
          double.tryParse(json['store_inventory_value']?.toString() ?? '0') ??
          0.0,
