...
class _FieldDetailScreenState extends ConsumerState<FieldDetailScreen> {
                    ...
                    children: [
                      Consumer(
                        builder: (context, ref, _) {
                          final listAsync = ref.watch(fieldListProvider);
                          return listAsync.maybeWhen(
                            data: (list) {
                              if (list.length <= 1) return const SizedBox.shrink();
                              final hasCurrent = list.any(
                                (item) => item.field.id == widget.fieldId,
                              );
                      ...
                      if (activeUpgrade != null) ...[
                        ...
                        _ActiveFieldUpgradeCard(
                          ...
                          onReduceTimeWithAd: () =>
                              _reduceFieldUpgradeTimeWithAd(activeUpgrade),
    ...
    await showBuildingUpgradeSheet(
      ...
      benefits: [
        BuildingUpgradeBenefit(
          ...
          before: '${detail.field.inputCapacity}',
  ...
  Future<void> _reduceFieldUpgradeTimeWithAd(
      ...
      return ProductSelectionOption(
        ...
        title:
            ...
            (selectableProduct.hasPreferredBrand
                ...
                : ''),
        subtitle:
            'Saatlik üretim: ${(product.uretimAdedi * (1.0 + (slot.qualityLevel - 1) * 0.20) * _getCityProductBonus(detail.field.cityId, product.kategori)).toInt()}'
            '${_getCityProductBonus(detail.field.cityId, product.kategori) > 1.0 ? " (x${_getCityProductBonus(detail.field.cityId, product.kategori).toStringAsFixed(2)} Bölge)" : ""}',
        badgeText:
  ...
  Future<void> _showSellFieldDialog(BuildContext context) async {
    final detail = ref.read(fieldDetailProvider(widget.fieldId)).value;
    if (detail == null) return;
    final field = detail.field;

    final quote = await ref
        .read(fieldActionProvider)
        .sellField(fieldId: field.id, confirm: false);
    ...
    final shouldSell = await showDialog<bool>(
      ...
      builder: (dialogContext) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            Text(
              '${field.name} kalici olarak silinecek. Bu islem geri alinamaz.',
              style: AppTextStyles.body.standardCopyWith(
                color: AppColors.textPrimary,
                fontSize: AppTypography.bodyLarge,
                height: 1.35,
