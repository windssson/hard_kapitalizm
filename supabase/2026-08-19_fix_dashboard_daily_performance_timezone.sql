-- Fix Homepage Dashboard and Daily Production to consistently use Europe/Istanbul timezone date
-- This fixes the bug where store sales (+325K TL profit) were recorded in Europe/Istanbul date
-- while homepage dashboard was reading from UTC date (0 TL revenue / 0 TL store profit).
...
begin
  ...
  values (
    coalesce(p_production_date, timezone('Europe/Istanbul', now())::date),
    p_player_id,
    p_owner_kind,
    p_owner_id,
    p_product_id,
  ...
  do update set
    ...
    total_cost =
      public.player_daily_production_stats.total_cost + excluded.total_cost,
...
begin
  ...
  -- Üretim Maliyetleri (Europe/Istanbul gününe göre)
  ...
  -- 3. Net Kâr = Mağaza Kârı + Depo Toptan Satış Kârı - Lojistik Giderleri - Üretim Maliyeti
  v_today_net_profit :=
    v_today_store_profit + v_today_warehouse_sale_profit - v_today_logistics_cost - v_today_production_cost;
