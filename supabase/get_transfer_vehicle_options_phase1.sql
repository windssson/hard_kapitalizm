create or replace function public.get_transfer_vehicle_options(
  p_source_kind text,
  p_source_id uuid,
  p_target_kind text,
  p_target_id uuid,
...
begin
  ...
  if p_source_kind = 'warehouse_slot' then
    ...
    into v_source_city_id, v_product_id, v_unit_volume
  ...
  elsif p_source_kind = 'store_slot' then
    select
      s.city_id,
      ss.product_id,
      coalesce(p.birim_hacim, 0)
  ...
  elsif p_source_kind = 'production_inventory' then
    select
      case
    ...
    raise exception 'Desteklenmeyen kaynak turu: %', p_source_kind;
  end if;

  if p_target_kind = 'warehouse' then
    select city_id into v_target_city_id
    from public.warehouses
    where id = p_target_id;
  elsif p_target_kind = 'store_slot' then
    select s.city_id into v_target_city_id
    from public.store_slots ss
    join public.stores s on s.id = ss.store_id
    where ss.id = p_target_id;
  elsif p_target_kind = 'production_inventory' then
    select
      case
        when pi.owner_kind = 'factory' then fx.city_id
        when pi.owner_kind = 'farm' then fa.city_id
    ...
    raise exception 'Desteklenmeyen hedef turu: %', p_target_kind;
  end if;

  if v_source_city_id is null or v_target_city_id is null then
    raise exception 'Transfer icin sehir bilgisi cozulmedi.';
  end if;

  if coalesce(v_unit_volume, 0) <= 0 then
    v_unit_volume := 1;
  end if;

  if v_source_city_id = v_target_city_id then
    return query
    select
      ...
      false as is_rental,
      greatest(p_quantity, 1) as capacity,
      999 as speed_kmh,
      999 as current_fuel,
  ...
  join public.cities tc on tc.id = v_target_city_id
  where sc.id = v_source_city_id;
  ...
  with own_vehicles as (
    select
      ...
      (
        ...
        and coalesce(lv.capacity, 0) >= ceil(p_quantity * v_unit_volume)
      ...
      case
        ...
        when coalesce(lv.capacity, 0) < ceil(p_quantity * v_unit_volume) then 'Kapasite yetersiz'
  ...
  npc_vehicle as (
    select
      ...
      (coalesce(n.capacity, 0) >= ceil(p_quantity * v_unit_volume)) as can_select,
      case
        when coalesce(n.capacity, 0) < ceil(p_quantity * v_unit_volume) then 'Kapasite yetersiz'
...
grant execute on function public.get_transfer_vehicle_options(text, uuid, text, uuid, integer) to anon, authenticated, service_role;
