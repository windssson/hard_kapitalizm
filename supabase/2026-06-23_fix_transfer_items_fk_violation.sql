-- Fix foreign key constraint violations when starting transfers.
-- The bug occurs because the source warehouse slot is deleted (when quantity drops to 0) BEFORE
-- inserting the transfer item into logistics_transfer_items, which references source_warehouse_slot_id.
-- Since the slot is already gone, it violates the foreign key constraint.
-- By inserting the item first and then updating/deleting the slot, the constraint (which has ON DELETE SET NULL)
-- will be satisfied and correctly set the reference to NULL upon slot deletion.
...
begin
  ...
  if p_source_entity_kind = 'warehouse' then
    ...
    where w.id = p_source_entity_id and w.player_id = v_player_id
    for update;
  ...
  if p_target_entity_kind = 'warehouse' then
    ...
    from public.warehouses w
    join public.cities c on c.id = w.city_id
    where w.id = p_target_entity_id and w.player_id = v_player_id
    for update;
  ...
  if v_same_city then
    ...
    -- Calculate distance and finish time before inserting
    v_distance_km := round(
      (
        6371 * 2 * asin(
          sqrt(
            power(sin(radians(coalesce(v_target_warehouse.map_position_x, 0) - coalesce(v_source_warehouse.map_position_x, 0)) / 2), 2)
            + cos(radians(coalesce(v_source_warehouse.map_position_x, 0)))
            * cos(radians(coalesce(v_target_warehouse.map_position_x, 0)))
            * power(sin(radians(coalesce(v_target_warehouse.map_position_y, 0) - coalesce(v_source_warehouse.map_position_y, 0)) / 2), 2)
          )
        )
      )::numeric,
      2
    ...
    v_duration_seconds := greatest(60, ceil((greatest(v_distance_km, 1) / v_vehicle.speed_kmh) * 720)::integer);
  ...
  for v_item in select value from jsonb_array_elements(p_items) loop
    ...
    if p_target_entity_kind = 'warehouse' then
      ...
      if found then
        ...
        set pending_quantity = coalesce(pending_quantity, 0) + v_item_quantity,
            updated_at = v_now
        ...
        if found then
          ...
          update public.warehouse_slots
          set product_id = v_source_slot.product_id,
    ...
    -- 1. Insert into logistics_transfer_items first while the source_warehouse_slot is still present.
    ...
    -- 2. Then update/delete the source slot. If it gets deleted, foreign key constraint ON DELETE SET NULL handles it correctly.
    update public.warehouse_slots
  ...
  set product_id = v_header_product_id,
...
begin
  ...
  for v_item in select value from jsonb_array_elements(p_items) loop
    ...
    if v_item_source_kind = 'npc_market' then
      if v_npc_logistics_player_id is null then v_npc_logistics_player_id := public.get_npc_logistics_player_id(); end if;
