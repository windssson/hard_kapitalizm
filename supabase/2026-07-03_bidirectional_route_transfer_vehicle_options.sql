-- Redefine get_route_transfer_vehicle_options to enforce bidirectional route matches and exclude vehicles without routes.
...
begin
  ...
  with own_vehicles as (
    select
      ...
      greatest(60, ceil((greatest(v_distance_km, 1) / greatest(coalesce(lv.speed_kmh, 0), 1)) * 120))::integer as estimated_duration_seconds,
      (
  ...
  player_rental_vehicles as (
    ...
    left join public.logistics_vehicle_types lvt on lvt.id = lv.logistics_vehicle_type_id
    where lv.player_id <> v_player_id
      and lv.player_id <> v_npc_player_id
      and coalesce(lv.is_available_for_rent, false) = true
      and public.logistics_vehicle_matches_route(
        lv.route_city_a_id,
        lv.route_city_b_id,
  ...
  selectable_union as (
    select * from own_vehicles where own_vehicles.can_select
    union all
    select * from player_rental_vehicles where player_rental_vehicles.can_select
    union all
    select * from npc_vehicle where npc_vehicle.can_select
  )
  select * from selectable_union;

  if not found then
    if exists (
      select 1
      from public.get_npc_rental_vehicle_option(p_source_city_id, p_target_city_id, v_distance_km) n
      where coalesce(n.capacity, 0) < v_required_capacity
    ) then
      return query
      select
        ...
        null::boolean as is_rental,
        null::integer as capacity,
        null::integer as speed_kmh,
        null::integer as current_fuel,
        null::integer as fuel_capacity,
        null::numeric as fuel_rate,
        null::integer as condition,
        null::numeric as rental_price,
        null::numeric as distance_km,
        null::numeric as fuel_needed,
        null::numeric as condition_needed,
        null::numeric as rental_cost,
        null::numeric as fuel_cost,
        null::numeric as total_price,
        null::integer as estimated_duration_seconds,
        false as can_select,
        'Kapasite sebebiyle araç bulunamadı.'::text as disabled_reason;
    else
      return query
      select
        null::uuid as vehicle_id,
        ...
        null::integer as estimated_duration_seconds,
        false as can_select,
        'Uygun araç bulunamadı.'::text as disabled_reason;
    end if;
  end if;
end;
$function$;
