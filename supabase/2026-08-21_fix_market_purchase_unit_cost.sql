-- 2026-08-21: Fix Buyer Unit Cost on Market Purchases
-- When a player buys from the wholesale market (another player or NPC),
-- the buyer's cost must be the purchase price (unit_price) paid by the buyer,
-- NOT the seller's internal production cost.
...
begin
  ...
  for v_item in select value from jsonb_array_elements(p_items) loop
    ...
    if v_item_source_kind = 'npc_market' then
      ...
      -- Buyer's cost is the purchase price paid for the goods!
      ...
      -- Buyer's cost is the purchase price paid for the goods (e.g. 750 TL), NOT the seller's internal cost!
      v_item_unit_cost := v_item_unit_price;
    ...
    -- 2. Then update/delete the seller slot.
    if v_item_source_kind <> 'npc_market' then
...
-- Fix the existing test account slot cost for KOYUN_ET
UPDATE public.warehouse_slots
SET cost = 750.0 + round(619.68 / 105.0, 4),
    updated_at = timezone('utc', now())
WHERE id = 'de41ae36-2768-4697-afe4-5a68bdbee9dc'
  AND product_id = 'KOYUN_ET';
