create or replace function public.generate_open_tenders(
  p_target_open_count integer default 8,
  p_max_generate integer default 8
)
returns jsonb
language plpgsql
security definer
...
begin
  ...
  v_generation_result := public.generate_open_tenders(8, 8);
  v_generated_count := coalesce((v_generation_result ->> 'generated_count')::integer, 0);
