-- 1. Redefine bootstrap_game_session to refresh leaderboard and company value upon login (splash screen)
...
begin
  ...
  -- Recalculate leaderboard stats and company value when the player logs in
...
-- 2. Redefine get_homepage_dashboard to bypass recalculation and read cached stats
...
begin
  ...
  -- 1. Performans İyileştirmesi: Önce leaderboard stats tablosundan önbellek verileri çekilir
  select
    coalesce(ls.achievement_unlocked_count, 0),
    coalesce(ls.achievement_total_count, 0),
    coalesce(ls.company_value, 0),
    true
  into
  ...
  -- Eğer veri bulunamadıysa (yeni oyuncu), ilk sefere mahsus tetikleriz
  if v_stats_exist is not true then
  ...
  -- Geri kalan dashboard bildirimlerini oluştur
