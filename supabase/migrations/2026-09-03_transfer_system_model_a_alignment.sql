-- ============================================================================
-- MIGRATION: 2026-09-03_transfer_system_model_a_alignment.sql
-- Model A: Lojistik / sevkiyat sisteminde mağaza depolarının kaldırılması,
-- hedeflerin Genel Depolar ve üretim girdi alanları olarak netleştirilmesi,
-- transfer tamamlama ve başlatma RPC'lerinin Genel Depo ile uyumlanması.
-- ============================================================================

-- ----------------------------------------------------------------------------
-- 1. get_city_consolidated_transfer_targets: Mağazalar hedef listesinden kaldırıldı
-- ----------------------------------------------------------------------------
...
BEGIN
  ...
  WITH all_targets AS (
    -- Depolar (Genel Depolar ve Özel Depolar)
    SELECT
      w.id,
      w.name,
    ...
    WHERE w.player_id = v_player_id
      ...
      AND (w.warehouse_kind IS NULL OR w.warehouse_kind IN ('normal', 'general'))
...
-- ----------------------------------------------------------------------------
-- 2. start_city_consolidated_transfer: Hedef 'store' ise Genel Depoya yönlendir
-- ----------------------------------------------------------------------------
...
BEGIN
  ...
  IF v_target_buyer_entity_kind = 'warehouse' AND v_target_warehouse.id IS NOT NULL THEN
...
-- ----------------------------------------------------------------------------
-- 3. complete_logistics_transfer_internal: 'store' hedefi Genel Depoya yönlendirildi
-- ----------------------------------------------------------------------------
...
BEGIN
  ...
  -- Eğer hedef 'store' olarak işaretlenmiş eski bir transfer varsa, mağazanın bulunduğu ildeki Genel Depo'ya teslim et
  IF coalesce(v_transfer.buyer_entity_kind, '') = 'store' THEN
    SELECT w.id
    INTO v_target_store_warehouse_id
    FROM public.stores s
    JOIN public.warehouses w ON w.player_id = s.player_id AND w.city_id = s.city_id
    WHERE s.id = v_transfer.buyer_store_id
      AND (w.warehouse_kind IS NULL OR w.warehouse_kind IN ('general', 'normal'))
      AND w.is_active = true
    ...
    IF NOT FOUND THEN
      v_target_store_warehouse_id := v_transfer.buyer_warehouse_id;
    END IF;

    IF v_target_store_warehouse_id IS NULL THEN
      RAISE EXCEPTION 'Hedef magazanin sehrinde Genel Depo bulunamadi.';
    END IF;
  END IF;
