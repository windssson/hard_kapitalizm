...
-- 16. assign_production_slot_product (Çiftlik/Tarla ürün kontrolü düzeltmesi & changed: { player })
...
-- 17. change_production_slot_product (Çiftlik/Tarla ürün kontrolü düzeltmesi & changed: { player })
...
-- 18. handle_logistics_transfer_notification (Anlık transferlerde bildirim oluşturmama)
...
begin
  -- Anlık transfer kontrolü:
  -- Mesafe 0 ise veya transfer başlangıcı ile bitişi arasında 10 saniyeden az süre varsa
  if coalesce(new.distance_km, 0) <= 0
     or coalesce(new.finish_at, new.started_at) <= new.started_at + interval '10 seconds'
     or coalesce(new.completed_at, new.started_at) <= new.started_at + interval '10 seconds' then
    v_is_instant := true;
  end if;
...
-- 19. trg_push_notification_on_insert_func (Anlık olaylarda doğrudan bildirim gönderimi)
