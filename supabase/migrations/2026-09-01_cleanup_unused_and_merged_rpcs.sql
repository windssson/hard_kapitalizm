-- =========================================================================================
-- MIGRATION: 2026-09-01_cleanup_unused_and_merged_rpcs.sql
-- 1. get_homepage_dashboard fonksiyonu ile get_homepage_dashboard_v2 birleştirildi (hourly_income_estimate eklendi).
-- 2. get_homepage_dashboard_v2 kaldırıldı.
-- 3. Mükerrer / eski aşırı yüklemeler (overloads) kaldırıldı:
--    - start_multi_market_transfer(uuid, uuid, uuid, jsonb, uuid) [eski 5 parametreli tek satıcı]
--    - ensure_npc_rental_vehicle(uuid, uuid) [eski 2 parametreli sabit araç]
-- 4. Tamamen atıl / ölü fonksiyonlar kaldırıldı:
--    - start_catalog_building_upgrade(uuid, text, uuid)
--    - start_catalog_building_upgrade_impl(uuid, text, uuid)
--    - update_player_heartbeat()
-- =========================================================================================

-- 1. get_homepage_dashboard fonksiyonunu hourly_income_estimate ile güncelle
...
begin
  ...
  return jsonb_build_object(
    ...
    'hourly_income_estimate', public.get_player_hourly_income_estimate(v_player_id),
...
-- 2. get_homepage_dashboard_v2 fonksiyonunu kaldır
DROP FUNCTION IF EXISTS public.get_homepage_dashboard_v2();

-- 3. Mükerrer / eski aşırı yüklemeleri kaldır
-- Eski 5 parametreli tek satıcı market transfer fonksiyonu
DROP FUNCTION IF EXISTS public.start_multi_market_transfer(uuid, uuid, uuid, jsonb, uuid);

-- Eski 2 parametreli tek tip kiralık araç fonksiyonu
DROP FUNCTION IF EXISTS public.ensure_npc_rental_vehicle(uuid, uuid);

-- 4. Tamamen kullanılmayan (atıl) fonksiyonları kaldır
DROP FUNCTION IF EXISTS public.start_catalog_building_upgrade(uuid, text, uuid);
DROP FUNCTION IF EXISTS public.start_catalog_building_upgrade_impl(uuid, text, uuid);
DROP FUNCTION IF EXISTS public.update_player_heartbeat();
