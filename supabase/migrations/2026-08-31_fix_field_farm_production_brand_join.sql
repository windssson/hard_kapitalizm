-- Migration: Fix field/farm production matching wrong brand inventory and clean up orphaned unbranded rows
-- Date: 2026-08-31
...
-- Cleanup obsolete unbranded 0-quantity output inventory rows where the slot is already branded
...
WHERE lt.status = 'completed'
  AND EXISTS (
    ...
    WHERE pi.id = lt.seller_production_inventory_id
      AND pi.owner_kind IN ('field', 'farm')
      AND pi.inventory_type = 'output'
      AND pi.brand_id = '00000000-0000-0000-0000-000000000000'::uuid
      AND coalesce(pi.quantity, 0) = 0
  );

DELETE FROM public.production_inventory pi
WHERE pi.owner_kind IN ('field', 'farm')
  AND pi.inventory_type = 'output'
  AND pi.brand_id = '00000000-0000-0000-0000-000000000000'::uuid
  AND coalesce(pi.quantity, 0) = 0
  AND EXISTS (
    ...
    WHERE ps.owner_kind = pi.owner_kind
      ...
      AND ps.brand_id <> '00000000-0000-0000-0000-000000000000'::uuid
  )
  AND NOT EXISTS (
    SELECT 1 FROM public.logistics_transfers lt
    WHERE lt.seller_production_inventory_id = pi.id
       OR lt.buyer_production_inventory_id = pi.id
  )
  AND NOT EXISTS (
    SELECT 1 FROM public.logistics_transfer_items lti
    WHERE lti.target_production_inventory_id = pi.id
  );
